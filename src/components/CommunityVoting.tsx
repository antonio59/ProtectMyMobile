    ...
    <div className="bg-card rounded-lg  p-6 md:p-8">
      ...
      <div className="flex items-center justify-between mt-8 pt-6 border-t border-border">
        ...
        {currentStep < 6 || (currentStep === 6 && formData.had_phone_stolen !== 'yes') ? (
          <button onClick={handleNext} disabled={!canProceed(formData, currentStep)} className="px-6 py-2 bg-primary text-primary-foreground rounded-lg hover:bg-primary-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors">Next →</button>
        ) : (
          <button onClick={handleSubmit} disabled={!canProceed(formData, currentStep) || isSubmitting} className="px-6 py-2 bg-foreground text-background rounded-lg hover:bg-foreground disabled:opacity-50 disabled:cursor-not-allowed transition-colors">{isSubmitting ? 'Submitting...' : 'Submit Anonymously'}</button>
        )}
      </div>
    </div>
  );
