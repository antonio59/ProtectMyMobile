...
async function fetchWDTKSearchPage(
  query: string,
  page: number = 1,
): Promise<WDTKRequest[]> {
  ...
  try {
    ...
    const requestMatches = html.matchAll(
      /<a[^>]+href="(\/request\/[^"]+)"[^>]*>([^<]+)<\/a>/gi,
    );
...
function isRelevantRequest(title: string, authority: string): boolean {
  ...
  const hasPhoneKeyword = /mobile|phone|smartphone|device|handset/i.test(
    titleLower,
  );
