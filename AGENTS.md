...
## Project Snapshot
- Single Astro + React + TypeScript site with Convex backend, deployed as a Cloudflare Worker.
- Package manager: pnpm. Deploy via `pnpm run deploy` (builds + `wrangler deploy`); Convex functions in `convex/`.
- `src/worker.ts` is the Worker entrypoint: host canonicalization, security headers, and cron triggers (replaces former netlify.toml + netlify/functions).
...
- Worker entrypoint (headers, host redirects, crons): `src/worker.ts`; config in `wrangler.jsonc`
...
- Find cron job definitions: `rg -n "CRON_JOBS" src/worker.ts`
